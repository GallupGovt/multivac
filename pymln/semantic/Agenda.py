...
class Agenda(object):
	...
	def addAgendaAfterMergeClust(self, p1, p2):
		assert p1._clustIdx == p2._clustIdx

		clustIdx = p1._clustIdx

		if p1.getParPart() is not None and p2.getParPart() is not None:
			clustIdx1 = p1.getParPart()._clustIdx
			clustIdx2 = p2.getParPart()._clustIdx
		...
		chs1, chs2 = [x.getArguments() for x in (p1, p2)]
	...
	def addAgendaForNewClust(self, newClustIdx):
		prnids = Part.getClustPartRootNodeIds()[newClustIdx]

		if len(prnids) > 1:
			for prnid in prnids:
				p = Part.getPartByRootNodeId(prnid)

				for prnid2 in prnids:
					if Utils.compareStr(prnid, prnid2) <= 0:
						break
					p2 = Part.getPartByRootNodeId(prnid2)
					self.addAgendaAfterMergeClust(p, p2)
