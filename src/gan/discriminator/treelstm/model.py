...
# # module for QueryGAN discriminator, adapting ChildSumTreeLSTM
# class ChildSumTreeLSTM(nn.Module):
#     def __init__(self, in_dim, mem_dim):
#         super(ChildSumTreeLSTM, self).__init__()
#         self.in_dim = in_dim
...
#         self.fx = nn.Linear(self.in_dim, self.mem_dim)
#         self.fh = nn.Linear(self.mem_dim, self.mem_dim)
#         self.final = nn.Linear(self.mem_dim, self.mem_dim)
...
#         c, h = self.node_forward(inputs[tree.idx], child_c, child_h)
#         result = F.sigmoid(self.final(h), 1)
...
class QueryGAN_Discriminator(nn.Module):
    def __init__(self, vocab_size, in_dim, mem_dim, sparsity, freeze):
        ...
        if freeze:
            self.emb.weight.requires_grad = False
        self.childsumtreelstm = ChildSumTreeLSTM(in_dim, mem_dim)
        self.confidence = nn.Linear(mem_dim, 1)
