...
def predict(con, h, r, t, num_top_rel=10, threshold=.1):
    if sum([x == -1 for x in (h, r, t)]) > 1:
        ...
        for rel in rels:
            result.append((h, rel, t), con.predict_triple(h, rel, r, threshold))
    elif t == -1:
...
def get_answers(con, query, direc, model, num_top_rel=10, threshold=.1):
    """
    ...
    files = glob.glob('{}*tf*'.format(direc))
    ...
    if not files:
        ...
        con.set_import_files('{}model.vec.{}.tf'.format(direc, ifile))
    ...
    files = glob.glob('{}/*.txt'.format(direc))
    ...
    if len(query['subject']) > 0:
        ...
        if subj_scores.max() < threshold:
            ...
            subj_id = entities.Ent.iloc[subj_scores.idxmax()]
    ...
    if len(query['object']) > 0:
        ...
        if obj_scores.max() < threshold:
            ...
            # Potentially use np.argmax() for multiple max results
            obj_id = entities.Ent.iloc[obj_scores.idxmax()]

    if len(query['relation']) > 0:
        rel_scores = entities.Rel.apply(lambda x: cos_sim(x, query['relation']))

        if rel_scores.max() < threshold:
            rel_id = -1
        else:
            # Potentially use np.argmax() for multiple max results
            rel_id = entities.Rel.iloc[rel_scores.idxmax()]
